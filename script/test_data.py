...
def fetch_data(_symbol, _interval, _output_csv):
    _data = requests.get('https://api.binance.com/api/v3/klines?symbol=' + _symbol + '&interval=' + _interval).json()
...
def get_indicators(_df_kline, _output_csv):
    ...
    # set time index
    _df_indicators['t'] = pd.to_datetime(_df_indicators['t'], unit='ms')
...
def main(_symbol, _interval, _kline_limit, _run_output_dir):
    ...
    df_kline = fetch_data(_symbol, _interval, kline_output)
...
if __name__ == '__main__':
    ...
    # init output name
    _base_output_name = args.symbol + '_' + args.interval
    ...
    main(args.symbol, args.interval, args.limit, run_output_dir)
